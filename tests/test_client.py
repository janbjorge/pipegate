...
class TestHandleRequest:
    async def test_forwards_request_to_target(
        ...
        async with httpx.AsyncClient(
            ...
            await handle_request("http://localhost:9000", sample_request, http_client, ws_client)
    ...
    async def test_preserves_method_and_body(self, correlation_id: uuid.UUID, ws_client: AsyncMock) -> None:
        """POST body and method are forwarded correctly."""
        request = BufferGateRequest(
            correlation_id=correlation_id, url_path="submit",
            url_query=orjson.dumps([]).decode(), method="POST",
        ...
        async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as http_client:
            await handle_request("http://localhost:9000", request, http_client, ws_client)
    ...
    async def test_preserves_url_path(self, correlation_id: uuid.UUID, ws_client: AsyncMock) -> None:
        """Target URL is constructed from target + url_path."""
        request = BufferGateRequest(
            correlation_id=correlation_id, url_path="nested/path/here",
            url_query=orjson.dumps([]).decode(), method="GET",
            headers=orjson.dumps({}).decode(), body="",
        )
    ...
    async def test_preserves_query_params(self, correlation_id: uuid.UUID, ws_client: AsyncMock) -> None:
        """Query parameters are forwarded to the target."""
        request = BufferGateRequest(
            correlation_id=correlation_id, url_path="search",
            url_query=orjson.dumps([["q", "test"], ["page", "1"]]).decode(),
            method="GET", headers=orjson.dumps({}).decode(), body="",
        )
    ...
    async def test_preserves_duplicate_query_params(self, correlation_id: uuid.UUID, ws_client: AsyncMock) -> None:
    ...
    async def test_preserves_headers(self, correlation_id: uuid.UUID, ws_client: AsyncMock) -> None:
        """Custom headers are forwarded to the target."""
        request = BufferGateRequest(
            correlation_id=correlation_id, url_path="api",
            url_query=orjson.dumps([]).decode(), method="GET",
            headers=orjson.dumps({"x-custom": "value", "accept": "application/json"}).decode(),
            body="",
        )
    ...
    async def test_binary_body_forwarded_correctly(self, correlation_id: uuid.UUID, ws_client: AsyncMock) -> None:
        """Binary (non-UTF-8) request bodies must be forwarded without corruption."""
        binary_body = bytes(range(256))
        request = BufferGateRequest(
            correlation_id=correlation_id, url_path="upload",
            url_query=orjson.dumps([]).decode(), method="POST",
            headers=orjson.dumps({}).decode(),
    ...
    async def test_http_error_returns_504(self, sample_request: BufferGateRequest, ws_client: AsyncMock) -> None:
    ...
    async def test_target_timeout_returns_504(self, sample_request: BufferGateRequest, ws_client: AsyncMock) -> None:
...
class TestMain:
    async def test_receives_and_processes_request(self, sample_request: BufferGateRequest) -> None:
        ...
        ws_mock.recv = recv_side_effect  # type: ignore[assignment]
        ws_mock.send = send_side_effect  # type: ignore[assignment]
